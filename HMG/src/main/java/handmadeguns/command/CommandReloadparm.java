...
public class CommandReloadparm extends CommandBase implements ICommand{
...
//        }
        ...
        for (File apack : packlist) {
            ...
            for (int ii = 0; ii < filegun.length; ii++) {
                if (filegun[ii].isFile()) {
                    try {
                        HMGAddGunsNew.load(null,false, filegun[ii]);
        ...
        HMGPacketHandler.INSTANCE.sendToAll(new PacketReloadparm());
    }
