...
public class TankBaseLogic extends LogicsBase implements IbaseLogic ,MultiRiderLogics{
    ...
    public TankBaseLogic(EntityLiving entityLiving) {
    ...
    public void updateClient(){
        ...
        if(needStartSound){
            ...
            if(mc_Entity.worldObj.isRemote)proxy_HMVehicle.playsoundasVehicle(64, mc_Entity);
        }
        ...
        ismanual = !((mc_Entity instanceof Hasmode) && ((Hasmode) mc_Entity).standalone());
        if(ismanual && mc_Entity.riddenByEntity == proxy_HMVehicle.getEntityPlayerInstance()){
    ...
    public void updateServer(){
        ...
        ismanual = !((mc_Entity instanceof Hasmode) && ((Hasmode) mc_Entity).standalone());
        if(ismanual && mc_Entity.riddenByEntity != null){
            aimMainTurret_toAngle(mc_Entity.riddenByEntity.getRotationYawHead(), mc_Entity.riddenByEntity.rotationPitch);
        }
        control();
        ...
        rotationmotion*= mc_Entity.onGround?0.8f:idriveableVehicle.ishittingWater() ? 0.95:0.999;
        if(mc_Entity.onGround || (tankinfo.canControlonWater && idriveableVehicle.ishittingWater()))bodyrotationYaw += rotationmotion;
...
//            playerlook = Vec3.createVectorHelper(playerlook.xCoord * 256, playerlook.yCoord * 256, playerlook.zCoord * 256);
        ...
        
            {
                ...
                if(mc_Entity.onGround || (tankinfo.canControlonWater && idriveableVehicle.ishittingWater())) {
...
//                }
    ...
    public boolean control(){
        ...
        if(ismanual && mc_Entity instanceof ITank && mc_Entity.riddenByEntity != null){
            if (servera) {
                rotationmotion -= (throttle + (tankinfo.spinturn && abs(throttle) == 0? 1 : 0)) * tankinfo.yawspeed * (mc_Entity.onGround? 1 : idriveableVehicle.ishittingWater() ? 0.5 : 0);
    ...
    public void moveFlying(float p_70060_1_, float front_speed, float p_70060_3_)
    {
        if(!ismanual) {
            ...
            if (idriveableVehicle.ishittingWater()) front_speed *= 0.4;
            if (mc_Entity.onGround || (tankinfo.canControlonWater && idriveableVehicle.ishittingWater())) {
    ...
    public boolean aimMainTurret_toAngle(double targetyaw, double targetpitch){
        if(iMultiTurretVehicle == null){
            if(idriveableVehicle.getMainTurret() != null) {
                idriveableVehicle.getMainTurret().currentEntity = mc_Entity;
            ...
            if (iMultiTurretVehicle.getmainTurrets() != null) {
                for (TurretObj aturret : iMultiTurretVehicle.getmainTurrets()) {
                    aturret.currentEntity = mc_Entity;
...
//    }
    ...
    public float getsoundPitch(){
        return throttle /tankinfo.throttle_Max*tankinfo.soundpitch;
    }
    ...
    public boolean riddenbyPlayer(){
        return proxy_HMVehicle.getEntityPlayerInstance() == mc_Entity.riddenByEntity;
    }
