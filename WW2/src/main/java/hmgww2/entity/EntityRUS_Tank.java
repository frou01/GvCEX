...
public class EntityRUS_Tank extends EntityRUSBase
{
	// public int type;
	
    public EntityRUS_Tank(World par1World)
    {
        super(par1World);
	...
	public boolean attackEntityFrom(DamageSource source, float par2)
    {
		...
		if(entity != null){
			...
			{
				par2 = par2 * this.TankArmor(entity,par2, 0.75F, 1.0F, 1.0F, this.rotation) * this.AntiBullet(entity, par2,2);
				return super.attackEntityFrom(source, par2);
			}
			else{
				par2 = par2 * this.TankArmor(entity,par2, 0.5F, 1.2F, 1.2F, this.rotationYawHead) * this.AntiBullet(entity, par2,3);
				return super.attackEntityFrom(source, par2);
			}
		}else{
			return super.attackEntityFrom(source, par2);
				...
				if (this.server1) {
					if (cooltime > this.ammo1 && this.getRemain_L() > 0) {
							this.Weapon(3, player, 0, 0, 2.1, 1.8,this.posX,this.posY,this.posZ,
									...
									55, 2F, 1F, 3, 1, 0.025D, true);//1.57/1.5/3
						cooltime = 0;
			...
			// attacktask
			{
				if(cooltime > ammo1 && this.getRemain_L() > 0)
				{
					this.Attacktask(3, 1, 40D, "gvclib:textures/entity/BulletAAA.obj",
							...
							55, 2F, 1F, 3F, 1, 0.025D, 400, 0, true);
				}
			}
		} // 1
