...
public class AIattackOnCollide extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        {
            boolean cansee = !attacker.isPotionActive(Potion.blindness);
            if(cansee) {
                if (attacker instanceof IGVCmob)
                    cansee = ((IGVCmob) attacker).canSeeTarget(entitylivingbase);
            }
            if(cansee) {
                if (--this.field_75445_i <= 0) {
                    this.entityPathEntity = worldForPathfind.getEntityPathToXYZ(attacker, (int) entitylivingbase.posX + 1, (int) entitylivingbase.posY + 1, (int) entitylivingbase.posZ + 1, 60, true, false, false, true);
    ...
    {
        if(attacker.onGround || attacker.isWet()) this.attacker.getNavigator().setPath(this.entityPathEntity, this.speedTowardsTarget);
