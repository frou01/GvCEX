	...
	public void onUpdate() {
		...
		if (this.standalone()) {
			if (!worldObj.isRemote && rand.nextInt(100) == 1) {
				...
				if(flag) {
					GVCEntitySoldierMG entitySoldierMG = new GVCEntitySoldierMG(worldObj);
					entitySoldierMG.addRandomArmor();
					entitySoldierMG.setLocationAndAngles(this.posX, this.posY, this.posZ, 0, 0);
					worldObj.spawnEntityInWorld(entitySoldierMG);
					if (!pickupEntity(entitySoldierMG, 0)) entitySoldierMG.setDead();
				}
