...
//	public float subturretrotationPitch;
    ...
    Vector3d subturretpos = new Vector3d(-0.65,2.55,-0.3f);
    Vector3d cannonpos = new Vector3d(0,2.2D,-3.0);
    Vector3d turretpos = new Vector3d(0,0,0);

    public int remainarena = 3;
    ...
    public GVCEntityTankT90(World par1World)
    {
        super(par1World);
        ...
        mainTurret = new TurretObj(worldObj);
        {
            ...
            mainTurret.powor = 200;
            mainTurret.ex = 5.0F;
            mainTurret.firesound = "gvcmob:gvcmob.125mmFire";
            mainTurret.firesoundLV = 5;
            mainTurret.spread = 1;
            mainTurret.speed = 16;
...
//	}
    ...
    protected void dropFewItems(boolean par1, int par2)
    {
        ...
        if(var3 == 0){
            this.entityDropItem(new ItemStack(GVCMobPlus.fn_pmctankT90, var3), 0.0F);
        }
        else {
    ...
    public void onUpdate()
    {
        ...
        if(!this.worldObj.isRemote){
            ...
            if(remainarena > 0) {
                List RPGdetector = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(6, 3.0D, 6));
                if (RPGdetector != null && !RPGdetector.isEmpty()) {
                    for (int i = 0; i < RPGdetector.size(); ++i) {
                        Entity colliedentity = (Entity) RPGdetector.get(i);
                        if (colliedentity instanceof HMGEntityBulletRocket && ((HMGEntityBulletRocket) colliedentity).thrower != this) {
                            double speed = colliedentity.motionX * colliedentity.motionX + colliedentity.motionY * colliedentity.motionY + colliedentity.motionZ * colliedentity.motionZ;
                            if (speed < 15) {
                                remainarena--;
                                ((HMGEntityBulletRocket) colliedentity).explode(colliedentity.posX, colliedentity.posY, colliedentity.posZ, ((HMGEntityBulletRocket) colliedentity).ex, ((HMGEntityBulletRocket) colliedentity).canex && cfg_blockdestroy && cfg_blockdestory);
                                colliedentity.setDead();
                            }
                        }
                        if (colliedentity instanceof HMGEntityBulletExprode && ((HMGEntityBulletExprode) colliedentity).thrower != this) {
                            double speed = colliedentity.motionX * colliedentity.motionX + colliedentity.motionY * colliedentity.motionY + colliedentity.motionZ * colliedentity.motionZ;
                            if (speed < 15) {
                                remainarena--;
                                ((HMGEntityBulletExprode) colliedentity).explode(colliedentity.posX, colliedentity.posY, colliedentity.posZ, ((HMGEntityBulletExprode) colliedentity).ex, ((HMGEntityBulletExprode) colliedentity).canex && cfg_blockdestroy && cfg_blockdestory);
                                colliedentity.setDead();
                            }
                        }
                    }
...
//    }
    ...
    public void readEntityFromNBT(NBTTagCompound p_70037_1_)
    {
        ...
        remainarena 						= p_70037_1_.getInteger("remainarena");
    }
    /**
    ...
    public void writeEntityToNBT(NBTTagCompound p_70014_1_)
    {
        ...
        p_70014_1_.setInteger("remainarena",remainarena);
    }
