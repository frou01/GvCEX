...
public class GVCEntityWZ10AttackHeli extends Entity implements ImultiRideableVehicle  {
    // public int type;
    ...
    public GVCEntityChild[] childEntities = new GVCEntityChild[2];
    public ChildInfo[] childInfo = new ChildInfo[2];
    boolean isinit;
    ...
    public GVCEntityWZ10AttackHeli(World par1World)
    {
        ...
        turret = new TurretObj(worldObj);
        {
            ...
            turret.turretspeedY = 30;
            turret.turretspeedP = 30;
            turret.powor = 26;
            turret.spread = 8;
            turret.ex = 0.1f;
            turret.canex = false;
    ...
    public boolean attackEntityFrom(DamageSource source, float par2)
    {
        ...
        if(childEntities[0] != null && childEntities[0].riddenByEntity != null)childEntities[0].riddenByEntity.attackEntityFrom(source, 0);
        health -=par2;
    ...
    public void onUpdate()
    {
        ...
        if(worldObj.isRemote){
            ...
            if(childEntities[0] != null && childEntities[0].riddenByEntity instanceof EntityGBase){
                control();
                if(childEntities[0].riddenByEntity.isDead)this.setDead();
            }
    ...
    void control(){
        EntityGBase pilot = (EntityGBase) childEntities[0].riddenByEntity;
        ...
        if (abs(pitchladder) > 0.001) {
            ...
            AxisAngle4d axisxangled = new AxisAngle4d(axisx, toRadians(-pitchladder / 3));
        ...
        if (abs(yawladder) > 0.001) {
            ...
            AxisAngle4d axisyangled = new AxisAngle4d(axisy, toRadians(yawladder / 2));
    ...
    void turret(Vector3d mainwingvector,Vector3d tailwingvector,Vector3d bodyvector){
        if(childEntities[1] != null && childEntities[1].riddenByEntity instanceof EntityLiving) {
            EntityLiving gunner = (EntityLiving) childEntities[1].riddenByEntity;
            if (!worldObj.isRemote && childEntities[0] != null && childEntities[0].riddenByEntity instanceof EntityLiving) {
                gunner.setAttackTarget(((EntityLiving) childEntities[0].riddenByEntity).getAttackTarget());
                if (((EntityLiving) childEntities[0].riddenByEntity).getAttackTarget() != null)
                    gunner.getLookHelper().setLookPositionWithEntity(((EntityLiving) childEntities[0].riddenByEntity).getAttackTarget(), 180, 180);
    ...
    void FCS(Vector3d mainwingvector,Vector3d tailwingvector,Vector3d bodyvector){
        if (trigger2) {
            if (rocket > 0) {
                rocket--;
                for(int i = 2;i<4;i++){
                    HMGEntityBulletRocket var3 = new HMGEntityBulletRocket(this.worldObj, childEntities[0].riddenByEntity, 100, 8, 8, 0.5F, GVCMobPlus.cfg_blockdestory);
                    var3.gra = 0.1f;
    ...
    void seatUpdate(Vector3d mainwingvector,Vector3d tailwingvector,Vector3d bodyvector){
        for(int i = 0; i< childEntities.length; i++){
            ...
            if(achild != null) {
                ...
                if(!worldObj.isRemote && achild.riddenByEntity==null && (rand.nextInt(50) == 0)){
                    EntityGBase newRider = null;
                    newRider = new GVCEntityGuerrilla(worldObj);
