...
//            playerlook = Vec3.createVectorHelper(playerlook.xCoord * 256, playerlook.yCoord * 256, playerlook.zCoord * 256);
...

            {
                ...
                bodyrotationPitch *= 0.99 * (tank.isInWater() ? 0.9:1);
                bodyrotationRoll *= 0.99 * (tank.isInWater() ? 0.9:1);
                if(tank.onGround || (canControlonWater && tank.isInWater())) {
...
//                }
        ...
        if(ismanual && tank instanceof IRideableTank && tank.riddenByEntity != null){
            if (servera) {
                ...
                rotationmotion -= (throttle + (spinturn && abs(throttle) == 0? 1 : 0)) * yawspeed * (tank.onGround? 1 : tank.isInWater() ? 0.5 : 0);
                throttle *= 0.8f;
        ...
        rotationmotion*=tank.isInWater()?0.95:tank.onGround ? 0.8f:0.999;
        if(tank.onGround || (canControlonWater && tank.isInWater()))bodyrotationYaw += rotationmotion;
...
//			System.out.println("debug" + throttle);
    ...
    public void movebyPlayer(float p_70060_1_, float p_70060_2_, float p_70060_3_)
    {
...
//			System.out.println("debug" + throttle);
    ...
    public boolean aimtoVector(Vector3d targetVec){
        {
            return aimMainTurret_toAngle(180-toDegrees(atan2(targetVec.x,targetVec.z)) , toDegrees(asin(targetVec.y)));
        }
    }
