...
public class GVCRenderPlane extends Render {
	...
	*/
	public void doRender(GVCEntityPlane entity, double p_76986_2_, double p_76986_4_, double p_76986_6_,
		...
		for(int mid = 1;mid <= entity.rocket;mid++){
			model.renderPart("missile" + mid);
		}

		GL11.glPushMatrix();
		GL11.glTranslatef(1.9927f, 1.5105f, -10.7406f);
		GL11.glRotatef(entity.rollladder,120.7f,-2.89f,-30.68f);
		GL11.glTranslatef(-1.9927f, -1.5105f, 10.7406f);
		...
		GL11.glRotatef(-10 * (float)entity.flaplevel/100,119.64f,-0.33f,0);
		GL11.glTranslatef(-0.6834f, -1.5139f, 10.8821f);
		...
		GL11.glRotatef(entity.pitchladder*3,125.85f,4.84f,-98.44f);
		GL11.glTranslatef(-0.4882f, -1.5925f, 14.0326f);
		...
		GL11.glRotatef(entity.yawladder*3,0,140.98f,-130.03f);
		GL11.glTranslatef(0, -2.4143f, 14.0213f);
		model.renderPart("YawLadder");
		GL11.glPopMatrix();

		if(entity.gearprogress > 10) {
			GL11.glPushMatrix();
			GL11.glTranslatef(1.6891f, 1.4900f, -9.4107f);
			GL11.glRotatef(-90 * (100 - entity.gearprogress) / 100, 1, 0, 1);
			GL11.glTranslatef(-1.6891f, -1.4900f, 9.4107f);
			...
			GL11.glRotatef(-90 * (100 - entity.gearprogress) / 100, 0, 0, 1);
			GL11.glTranslatef(-0.3463f, -1.0753f, 8.1108f);
		...
		GL11.glRotatef(-90 * (100 - entity.gearprogress) / 100, 1, 0, 0);
		GL11.glTranslatef(0, -0.9934f, 4.5152f);
		...
		GL11.glRotatef(-105 * (100 - entity.gearprogress) / 100, 0, 0.08715574274f, 0.99619469809f);
		GL11.glTranslatef(-0.1606f, -1.0239f, 3.918f);
		...
		GL11.glRotatef((entity.pitchladder), 1, 0, 0);
		GL11.glRotatef((entity.rollladder), 0, 0, 1);
		GL11.glTranslatef(0, -1.4627f, 4.3287f);
		model.renderPart("Lever");
		GL11.glPopMatrix();

		if(entity.throttle>9.5){
			float lastBrightnessX = OpenGlHelper.lastBrightnessX;
			float lastBrightnessY = OpenGlHelper.lastBrightnessY;
			OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240, 240);
			GL11.glColor4f(1,1,1,(entity.throttle-9.5f)*2);
